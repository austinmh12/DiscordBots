...
import sys
import typing
from mudae_cogs.waifus import Waifus
from mudae_cogs.pokemons import Pokemons
from mudae_cogs.minecraft import Minecraft
from mudae_cogs.pokeroulette import PokeRoulette

with open('../.env') as f:
	ENV = {l.strip().split('=')[0]: l.strip().split('=')[1] for l in f.readlines()}

SCRIPT_PATH = 'S:/OldPC/Python/Bots/discord_bots/Anime' if sys.platform == 'win32' else '/home/austinmh12/Documents/Code/Python/discord_bots/Anime'
...
# waifu_cog = Waifus(client)
# client.add_cog(waifu_cog)
client.add_cog(Pokemons(client))
mc_cog = Minecraft(client)
client.add_cog(mc_cog)
client.add_cog(PokeRoulette(client))
# client.loop.create_task(mc_cog.ping())
client.run(ENV['MUDAETOKEN'], bot=True)
